/// <reference path='../../bower_components/dt-jquery/jquery.d.ts' />
/// <reference path='../../bower_components/dt-angular/angular.d.ts' />
/// <reference path='../../bower_components/dt-angular/angular-resource.d.ts' />
/// <reference path='../../bower_components/dt-angular/angular-route.d.ts' />
/// <reference path='../../bower_components/dt-d3/d3.d.ts' />

//################## Controllers ###################
/// <reference path='controllers/iLivingCtrl.ts' />
/// <reference path='controllers/timedUsageCtrl.ts' />
/// <reference path='controllers/navCtrl.ts' />
/// <reference path='controllers/applianceUsageCtrl.ts' />
/// <reference path='directives/stackedAreaDirective.ts' />
/// <reference path='directives/barGraphDirective.ts' />

//################# App ###########################
/// <reference path='app.ts' />

