...
module iLiving.directives{
  ...
  export function barGraph() : ng.IDirective{
	...
	scope.$watch('devicesconsumption', (newDevicesConsumption : Array<DeviceConsumption>, oldDevicesConsumption) => {

	  var height = scope.height - margin.top - margin.bottom;
	  var width = scope.width - margin.left - margin.right;
	  svg.selectAll('*').remove();

	  if(!newDevicesConsumption) return;
	...
	x.domain(newDevicesConsumption.map(function(d){ return d.name; }));
	y.domain([0, d3.max(newDevicesConsumption, function(d){ return d.consumption; })]);
	...
	bar.append("text")
	  ...
	  .attr("y", function(d){ return y(d.consumption) - 5 })
	  .attr("text-anchor", "middle")
	  .text(function(d){ return d.cost + "kr."; });
